...
const CourseCreator: React.FC<CourseCreatorProps> = ({ onCourseCreated }) => {
  ...
  const handleGenerate = async () => {
    ...
    const title = inputType === 'youtube' 
      ? 'AI Course: Machine Learning Fundamentals'
      : `AI Course: ${file?.name?.replace('.pdf', '') || 'Document Analysis'}`;
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600">
            Transform any YouTube video or PDF into an interactive learning experience
          </p>
        </div>
          ...
          {inputType === 'youtube' && (
            <div className="space-y-6">
              ...
              <div className="bg-blue-50 p-4 rounded-xl">
                <h4 className="font-medium text-blue-900 mb-2">Sample URLs to try:</h4>
                <div className="space-y-1 text-sm text-blue-700">
                  <div>• Machine Learning: https://youtube.com/watch?v=aircAruvnKk</div>
                  <div>• Web Development: https://youtube.com/watch?v=UB1O30fR-EE</div>
                  <div>• Data Science: https://youtube.com/watch?v=r-uHLfm7DdU</div>
                </div>
              </div>
            </div>
          )}
